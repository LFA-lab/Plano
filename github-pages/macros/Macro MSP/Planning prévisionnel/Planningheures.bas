Sub Export_Plan_Et_HeuresMonteurs()
    Dim xlApp As Object
    ...
    Application.StatusBar = "Génération du planning prévisionnel, Injection des heures Monteurs..."
    Call Injecter_HeuresMonteurs_Dans_Planning(xlApp, xlWorkbook)
    
    Application.StatusBar = "Génération du plan de charge VE..."
    Call Ajouter_VE_Plandecharge_Comme_Premiere_Feuille(xlApp, xlWorkbook)
...
' Procédure de compatibilité - redirige vers la nouvelle version
Sub PlanningHeures()
    Call Export_Plan_Et_HeuresMonteurs
End Sub
...
Sub GenerateWeekView(ws As Object, allTasks As Variant, taskCount As Integer, projDoc As Object)
    ...
    Call AjouterTotalMonteursParSemaine(ws, weekDates, UBound(weekHeaders))
...
Sub AjouterTotalMonteursParSemaine(ws As Object, weekDates() As Date, maxWeeks As Integer)
    ' Cette fonction calcule et affiche le total des monteurs par semaine sur la ligne 3
    Dim i As Integer
    Dim weekNum As Integer
    Dim totalMonteurs As Double
    Dim projet As Project
    Dim tache As task, assign As assignment
    Dim d As Date
    Dim dictWeeklyTotal As Object
    
    ' Initialisation
    Set projet = ActiveProject
    Set dictWeeklyTotal = CreateObject("Scripting.Dictionary")
    ...
    For Each tache In projet.Tasks
        If Not tache Is Nothing And Not tache.Summary Then
            For Each assign In tache.Assignments
                If Not assign.Resource Is Nothing And assign.Resource.Name = "Monteurs" Then
                    For d = assign.Start To assign.Finish
                        If Weekday(d, vbMonday) <= 5 Then ' Seulement les jours ouvrables
                            weekNum = CInt(Format(d, "ww", vbMonday, vbFirstFourDays))
                            
                            If Not dictWeeklyTotal.Exists(weekNum) Then
                                dictWeeklyTotal.Add weekNum, 0
                            End If
                            
                            ' Ajouter les unités de monteurs (convertir en hommes)
                            dictWeeklyTotal(weekNum) = dictWeeklyTotal(weekNum) + assign.Units
                        End If
                    Next d
                End If
            Next assign
    ...
    ' Afficher les totaux sur la ligne 3
    ws.Cells(3, 1).Value = "Total Monteurs"
    ws.Cells(3, 2).Value = ""
    
    For i = 1 To maxWeeks
        If i <= UBound(weekDates) Then
            weekNum = CInt(Format(weekDates(i), "ww", vbMonday, vbFirstFourDays))
    ...
    ' Formatage de la ligne 3
    ws.Rows(3).Font.Bold = True
    ws.Rows(3).Interior.Color = RGB(255, 255, 200) ' Jaune clair
    ws.Cells(3, 1).Interior.Color = RGB(255, 255, 200)
...
Sub Injecter_HeuresMonteurs_Dans_Planning(xlApp As Object, xlBook As Object)
    Dim wsJours As Object
    Dim projet As Project
    Dim tache As task, assign As assignment
    ...
    ' Variables pour les heures Monteurs
    Dim heuresJour As Double
    Dim dateKey As Long, colIdx As Long
    Dim arrRowIdx As Long, arrColIdx As Long
    Dim currentValue As Double, somme As Double
    
    Debug.Print "Injection des heures Monteurs - DÉBUT"
    Set projet = ActiveProject
    ...
    If wsJours Is Nothing Then
        MsgBox "L'onglet 'Jours' n'existe pas dans le fichier Excel.", 16
        Exit Sub
    End If
    ...
    ' Construire le dictionnaire des dates
    ...
    ' Construire le dictionnaire des tâches
    ...
    ' Lire la plage de données en mémoire
    ...
    For Each tache In projet.Tasks
        If Not tache Is Nothing And Not tache.Summary Then
            ...
            If rowIdx.Exists(tacheName) Then
                ...
                ' Chercher les affectations "Monteurs"
                For Each assign In tache.Assignments
                    If Not assign.Resource Is Nothing Then
                        If assign.Resource.Name = "Monteurs" And assign.Resource.Type = pjResourceTypeWork Then
                            ...
                            If debut <= fin Then
                                ' Extraire les données temps-phasées pour Work (en heures)
                                Set tsData = assign.TimeScaleData(debut, fin, pjAssignmentTimescaledWork, pjTimescaleDays)
                                ...
                                For i = 1 To tsData.Count
                                    ...
                                    If IsNumeric(tsValue.Value) And tsValue.Value > 0 Then
                                        ' Conversion minutes ? heures (pjAssignmentTimescaledWork retourne en minutes)
                                        ...
                                        If dateIdx.Exists(dateKey) Then
                                            ...
                                            If arrRowIdx >= 1 And arrRowIdx <= UBound(arr, 1) And _
                                               arrColIdx >= 1 And arrColIdx <= UBound(arr, 2) Then
                                                ...
                                                ' Additionner les heures (multi-affectations)
    ...
    For col = 3 To lastCol
        ...
        If somme > 0 Then
            ' Diviser par 9 pour obtenir le besoin en monteurs par jour
            totauxArr(1, arrColIdx) = Round(somme / 9, 2)
        Else
            totauxArr(1, arrColIdx) = ""
        End If
    Next col
    ...
    Debug.Print "Injection des heures Monteurs - FIN (" & compteurValeurs & " valeurs injectées)"
End Sub
...
Sub VE_Plandecharge()
    ...
    Application.StatusBar = "G�n�ration du plan de charge - Initialisation Excel..."
    Set xlBook = xlApp.Workbooks.Add
    Set xlSheet = xlBook.Sheets(1)

    ' Logo + ent�te
    Application.StatusBar = "G�n�ration du plan de charge - Insertion du logo..."
    InsererLogoOmexom xlSheet
    
    Application.StatusBar = "G�n�ration du plan de charge - Compilation des donn�es..."
    ...
    With xlSheet.Range("A1:F1")
        ...
        .Font.Size = 16
        .Font.Bold = True
        .HorizontalAlignment = -4108 ' Centr� horizontalement
        .VerticalAlignment = -4108   ' Centr� verticalement
    ...
    xlSheet.Cells(3, 2).Value = "Date D�but"
    ...
    xlSheet.Cells(3, 6).Value = "Pic Hommes (jour)"

    ' Dictionnaires
    Dim dictWeeklyTotal As Object: Set dictWeeklyTotal = CreateObject("Scripting.Dictionary")
    Dim dictWeeklyDailyPeak As Object: Set dictWeeklyDailyPeak = CreateObject("Scripting.Dictionary")
    Dim dictWeeklyStart As Object: Set dictWeeklyStart = CreateObject("Scripting.Dictionary")
    Dim dictWeeklyFinish As Object: Set dictWeeklyFinish = CreateObject("Scripting.Dictionary")
    ...
    For Each t In ActiveProject.Tasks
        If Not t Is Nothing And Not t.Summary Then
            For Each a In t.Assignments
                If Not a.Resource Is Nothing And a.Resource.Name = "Monteurs" Then
                    For d = a.Start To a.Finish
                        If Weekday(d, vbMonday) <= 5 Then
                            weekNum = CInt(Format(d, "ww", vbMonday, vbFirstFourDays))

                            If Not dictWeeklyTotal.Exists(weekNum) Then
                                dictWeeklyTotal.Add weekNum, 0
                                dictWeeklyDailyPeak.Add weekNum, CreateObject("Scripting.Dictionary")
                                dictWeeklyStart.Add weekNum, d
                            ...
                            If Not dictWeeklyDailyPeak(weekNum).Exists(dayKey) Then
                                dictWeeklyDailyPeak(weekNum).Add dayKey, 0
                            End If
                            dictWeeklyDailyPeak(weekNum)(dayKey) = dictWeeklyDailyPeak(weekNum)(dayKey) + a.Units

                            If d < dictWeeklyStart(weekNum) Then dictWeeklyStart(weekNum) = d
                            If d > dictWeeklyFinish(weekNum) Then dictWeeklyFinish(weekNum) = d
                        End If
                    Next d
                End If
            Next a
    ...
    For Each week In dictWeeklyTotal.Keys
        maxUnits = 0: maxDay = ""
        For Each day In dictWeeklyDailyPeak(week).Keys
            If dictWeeklyDailyPeak(week)(day) > maxUnits Then
                maxUnits = dictWeeklyDailyPeak(week)(day)
                maxDay = day
            End If
        Next day

        xlSheet.Cells(rowOut, 1).Value = week
        xlSheet.Cells(rowOut, 1).NumberFormat = """Semaine ""00"
    ...
    ' Tri num�rique sur la colonne Semaine (valeurs r�elles)
    With xlSheet.Sort
        .SortFields.Clear
        .SortFields.Add xlSheet.Range("A4:A" & rowOut - 1), 0, 1
        .SetRange xlSheet.Range("A3:F" & rowOut - 1)
        .Header = 1 ' xlYes
        .MatchCase = False
...
Sub Ajouter_VE_Plandecharge_Comme_Premiere_Feuille(xlApp As Object, xlWorkbook As Object)
    ' Cette sous-routine ajoute le plan de charge VE comme première feuille du classeur existant
    Dim wsPlanCharge As Object
    Dim wsExistante As Object
    
    On Error GoTo ErrorHandler
    
    ' Créer une nouvelle feuille au début du classeur
    Set wsPlanCharge = xlWorkbook.Worksheets.Add(Before:=xlWorkbook.Worksheets(1))
    wsPlanCharge.Name = "Plan de Charge VE"
    
    ' Générer le contenu du plan de charge directement dans cette feuille
    Call Generer_Contenu_VE_Plandecharge(wsPlanCharge)
    
    Exit Sub
    
ErrorHandler:
    MsgBox "Erreur lors de l'ajout du plan de charge VE : " & Err.Number & " - " & Err.Description, vbCritical
End Sub

Sub Generer_Contenu_VE_Plandecharge(xlSheet As Object)
    ' Cette sous-routine génère le contenu du plan de charge VE dans une feuille spécifiée
