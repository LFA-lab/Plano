Attribute VB_Name = "EcartHeuresParTacheExportExcel"
Sub EcartHeures()
    Dim tache As task
    Dim xlApp As Object, xlBook As Object
    Dim xlSheet As Object
    Dim feuilleMicro As Object
    Dim ligneMacro As Long, ligneMicro As Long
    Dim baseline As Double, prevu As Double, realise As Double
    Dim ecartPrevu As Double, ecartRef As Double, ecartPct As Double
    Dim rowIndex As Long
    Dim expTexte As String, expColor As Long
    Dim i As Long
    Dim headersMacro As Variant, headersMicro As Variant

    Application.StatusBar = "D�marrage Excel..."
    Set xlApp = CreateObject("Excel.Application")
    Set xlBook = xlApp.Workbooks.Add
    xlApp.Visible = False

    Set xlSheet = xlBook.Sheets(1)
    xlSheet.Name = "Vue macro"
    Application.StatusBar = "Insertion du logo sur la vue macro..."
    InsererLogoOmexom xlSheet

    Set feuilleMicro = xlBook.Sheets.Add(After:=xlSheet)
    feuilleMicro.Name = "Vue micro"
    Application.StatusBar = "Insertion du logo sur la vue micro..."
    InsererLogoOmexom feuilleMicro

    Application.StatusBar = "Mise en place des en-t�tes..."
    ' En-t�te bleu fusionn� (A1:H1 pour macro, A1:G1 pour micro) + titre
    ...
    With feuilleMicro.Range("A1:G1")
        .Merge
        .Value = "Bilan d�taill� des heures - Monteurs"
        .Font.Size = 16
        .Font.Bold = True
        .HorizontalAlignment = -4108
        .VerticalAlignment = -4108
    ...
    headersMacro = Array("Nom de la t�che", "Travail r�f. (h)", "Travail pr�vu (h)", "Travail r�alis� (h)", _
                         "? vs pr�vu (h)", "? vs r�f. (h)", "R�sultat (%)", "Explication")
    headersMicro = Array("Nom de la t�che", "Travail r�f. (h)", "Travail pr�vu (h)", "Travail r�alis� (h)", _
                         "? vs pr�vu (h)", "? vs r�f. (h)", "R�sultat (%)")
    ...
    feuilleMicro.Rows(2).Font.Bold = True

    ligneMacro = 3
    ligneMicro = 3

    Application.StatusBar = "Compilation des donn�es du projet..."
    For Each tache In ActiveProject.Tasks
        If Not tache Is Nothing Then
            baseline = tache.BaselineWork / 60
            prevu = tache.Work / 60
            realise = tache.actualWork / 60
            ecartPrevu = realise - prevu
            ecartRef = realise - baseline
            If baseline > 0 Then
                ecartPct = (realise - baseline) / baseline
            Else
                ecartPct = 0
            End If

            If ecartPct >= 0.5 Or ecartPct <= -0.5 Then
                expTexte = "CRITIQUE"
                expColor = RGB(255, 80, 80) ' Rouge
            ElseIf ecartPct > 0 Then
                expTexte = "DEPASSEMENT"
                expColor = RGB(255, 192, 0) ' Orange
            ElseIf ecartPct < 0 Then
                expTexte = "SOUS-BUDGET"
                expColor = RGB(0, 176, 80) ' Vert fonc�
            Else
                expTexte = "CONFORME"
                expColor = RGB(146, 208, 80) ' Vert clair
            End If

            If tache.Summary Then
                rowIndex = ligneMacro: ligneMacro = ligneMacro + 1
                xlSheet.Cells(rowIndex, 1).Value = tache.Name
                xlSheet.Cells(rowIndex, 2).Value = baseline
                xlSheet.Cells(rowIndex, 3).Value = prevu
                xlSheet.Cells(rowIndex, 4).Value = realise
                xlSheet.Cells(rowIndex, 5).Value = ecartPrevu
                xlSheet.Cells(rowIndex, 6).Value = ecartRef
                xlSheet.Cells(rowIndex, 7).Value = ecartPct
                xlSheet.Cells(rowIndex, 8).Value = expTexte
                xlSheet.Cells(rowIndex, 8).Interior.Color = expColor
                xlSheet.Cells(rowIndex, 8).Font.Bold = True
            Else
                rowIndex = ligneMicro: ligneMicro = ligneMicro + 1
                feuilleMicro.Cells(rowIndex, 1).Value = tache.Name
                feuilleMicro.Cells(rowIndex, 2).Value = baseline
                feuilleMicro.Cells(rowIndex, 3).Value = prevu
                feuilleMicro.Cells(rowIndex, 4).Value = realise
                feuilleMicro.Cells(rowIndex, 5).Value = ecartPrevu
                feuilleMicro.Cells(rowIndex, 6).Value = ecartRef
                feuilleMicro.Cells(rowIndex, 7).Value = ecartPct
            End If
        End If
    Next tache

    Application.StatusBar = "Mise en forme des colonnes..."
    xlSheet.Columns("B:F").NumberFormat = "0.0 ""h"""
    xlSheet.Columns("G:G").NumberFormat = "0.0%"
    xlSheet.Columns("H:H").NumberFormat = "@"
    feuilleMicro.Columns("B:F").NumberFormat = "0.0 ""h"""
    feuilleMicro.Columns("G:G").NumberFormat = "0.0%"
...
Public Sub EcartHeures_Bouton()
    ' Lance l'analyse des écarts d'heures par tâche
    EcartHeures
End Sub
...
Public Sub InstallerBoutonEcartHeures()
    ...
    With btn
        .Caption = "Écart Heures"
        .OnAction = "EcartHeures_Bouton"  ' appelle le wrapper
        .Style = msoButtonIconAndCaption
        .FaceId = 433  ' icône horloge/temps
        .TooltipText = "Analyser les écarts d'heures par tâche - Monteurs"
    End With
