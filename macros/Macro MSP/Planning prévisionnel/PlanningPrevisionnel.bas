...
Sub Injecter_UnitesDePointe_Dans_Planning(xlApp As Object, xlBook As Object)
    ...
    For Each tache In projet.Tasks
        If Not tache Is Nothing And Not tache.Summary Then
            ...
            If idxTache = -1 Then GoTo NextTask
                                ...
                                For col = 3 To lastCol
                                    If IsDate(wsJours.Cells(100, col).Value) Then
                                        If DateValue(wsJours.Cells(100, col).Value) = DateValue(dateJour) Then
                                            If IsNumeric(wsJours.Cells(idxTache, col).Value) Then
                                                wsJours.Cells(idxTache, col).Value = wsJours.Cells(idxTache, col).Value + Round(CDbl(tsValue.Value), 2)
                                            ElseIf wsJours.Cells(idxTache, col).Value = "" Then
                                                wsJours.Cells(idxTache, col).Value = Round(CDbl(tsValue.Value), 2)
                                            End If
...
NextTask:
    ...
    For col = 3 To lastCol
        ...
        somme = 0
        For ligne = 5 To lastRow
            If IsNumeric(wsJours.Cells(ligne, col).Value) Then
                somme = somme + wsJours.Cells(ligne, col).Value
            End If
        Next ligne
        If somme > 0 Then
            wsJours.Cells(3, col).Value = somme
...
Public Sub PlanningPrevisionnel_Bouton()
    ' Lance l'export du planning prévisionnel avec unités de pointe
    Export_Plan_Et_UnitesDePointe
End Sub

' Crée un bouton dans l'onglet Compléments (Add-Ins) pour lancer l'export
Public Sub InstallerBoutonPlanningPrevisionnel()
    On Error Resume Next
    ' Nettoyage si déjà présent
    Application.CommandBars("PlanningPrevisionnel").Delete
    On Error GoTo 0

    Dim cb As CommandBar
    Dim btn As CommandBarButton

    Set cb = Application.CommandBars.Add(Name:="PlanningPrevisionnel", Position:=msoBarTop, Temporary:=True)
    Set btn = cb.Controls.Add(Type:=msoControlButton)

    With btn
        .Caption = "Planning Prévisionnel"
        .OnAction = "PlanningPrevisionnel_Bouton"  ' appelle le wrapper
        .Style = msoButtonIconAndCaption
        .FaceId = 590  ' icône planning/calendrier
        .TooltipText = "Générer le planning prévisionnel avec unités de pointe"
    End With
...
Public Sub SupprimerBoutonPlanningPrevisionnel()
    On Error Resume Next
    Application.CommandBars("PlanningPrevisionnel").Delete
    On Error GoTo 0
End Sub
