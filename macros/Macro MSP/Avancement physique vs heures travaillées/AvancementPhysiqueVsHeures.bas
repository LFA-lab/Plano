Attribute VB_Name = "PhysiqueVsHeures"
Sub AvancementPhysiqueVsHeures()

    Dim tache As task
    Dim assignment As assignment
    Dim physiquePct As Double
    Dim heuresPct As Double
    Dim physiqueTrouv� As Boolean
    Dim travailTrouv� As Boolean
    Dim ecart As Double
    ...
    ' Initialisation Excel en mode cach�
    ...
    With xlSheet.Range("A1:F1")
        .Merge
        .Value = "Avancement physique vs heures r�alis�es"
        .Font.Size = 16
        .Font.Bold = True
    ...
    ' En-t�tes du tableau
    xlSheet.Cells(2, 1).Value = "Travail pr�vu (ex. Pose de c�ble)"
    xlSheet.Cells(2, 2).Value = "Date de d�but"
    xlSheet.Cells(2, 3).Value = "% Avancement Physique (mat�riaux pos�s)"
    xlSheet.Cells(2, 4).Value = "% Avancement des Heures (heures pass�es)"
    xlSheet.Cells(2, 5).Value = "�cart d�efficacit� (%)"
    ...
    Application.StatusBar = "Export en cours�: g�n�ration du bilan des heures..."
    ...
    For Each tache In ActiveProject.Tasks
        If Not tache Is Nothing And Not tache.Summary Then
            ...
            If physiqueTrouv� And travailTrouv� Then

                ecart = heuresPct - physiquePct

                ' Statut selon priorit�
                ...
                ElseIf tache.Start > today And physiquePct = 0 And heuresPct = 0 Then
                    statut = "� venir"
                    xlSheet.Range("A" & ligne & ":F" & ligne).Interior.Color = RGB(221, 235, 247)    ' Bleu tr�s clair

                Else
                    If ecart > 5 Then
                        statut = "Trop lent / En retard"
                        xlSheet.Range("A" & ligne & ":F" & ligne).Interior.Color = RGB(255, 204, 204)    ' Rouge clair
                    ElseIf ecart < -5 Then
                ...
                ' Remplir la ligne
                xlSheet.Cells(ligne, 1).Value = tache.Name
                xlSheet.Cells(ligne, 2).Value = tache.Start
                xlSheet.Cells(ligne, 3).Value = physiquePct / 100
                xlSheet.Cells(ligne, 4).Value = heuresPct / 100
                xlSheet.Cells(ligne, 5).Value = ecart / 100
                xlSheet.Cells(ligne, 6).Value = statut
    ...
    xlSheet.Range("C3:E" & ligne - 1).NumberFormat = "0.0%"
    xlSheet.Range("B3:B" & ligne - 1).NumberFormat = "dd/mm/yyyy"
    xlSheet.Range("F3:F" & ligne - 1).Font.Bold = True

    ' Fin du process
    xlApp.Visible = True
    Application.StatusBar = "Export termin�: " & (ligne - 3) & " t�ches export�es."
...
Public Sub InstallerBoutonAvancementPhysiqueVsHeures()
    ...
    With btn
        ...
        .FaceId = 618  ' icône analyse/graphique
        .TooltipText = "Analyser l'avancement physique vs heures travaillées"
    End With
