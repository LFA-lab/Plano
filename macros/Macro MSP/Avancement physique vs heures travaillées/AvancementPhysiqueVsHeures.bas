...
Sub AvancementPhysiqueVsHeures()
    ...
    Dim physiqueTrouvé As Boolean
    Dim travailTrouvé As Boolean
    Dim ecart As Double
    ...
    ' Initialisation Excel en mode caché
    ...
    With xlSheet.Range("A1:F1")
        .Merge
        .Value = "Avancement physique vs heures réalisées"
        .Font.Size = 16
        .Font.Bold = True
    ...
    ' En-têtes du tableau
    xlSheet.Cells(2, 1).Value = "Travail prévu (ex. Pose de câble)"
    xlSheet.Cells(2, 2).Value = "Date de début"
    xlSheet.Cells(2, 3).Value = "% Avancement Physique (matériaux posés)"
    xlSheet.Cells(2, 4).Value = "% Avancement des Heures (heures passées)"
    xlSheet.Cells(2, 5).Value = "Écart d’efficacité (%)"
    ...
    Application.StatusBar = "Export en cours : génération du bilan des heures..."
    ...
    For Each tache In ActiveProject.Tasks
        If Not tache Is Nothing And Not tache.Summary Then
            ...
            If physiqueTrouvé And travailTrouvé Then

                ecart = heuresPct - physiquePct

                ' Statut selon priorité
                ...
                ElseIf tache.Start > today And physiquePct = 0 And heuresPct = 0 Then
                    statut = "À venir"
                    xlSheet.Range("A" & ligne & ":F" & ligne).Interior.Color = RGB(221, 235, 247)    ' Bleu très clair
    ...
    ' Fin du process
    xlApp.Visible = True
    Application.StatusBar = "Export terminé : " & (ligne - 3) & " tâches exportées."
